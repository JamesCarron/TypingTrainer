...
class Game:
    ...
    def __init__(self, root, window_width=1200, window_height=700):
        ...
        self.root.tk.call('wm', 'iconphoto', root._w, tk.PhotoImage(file='Images/ico.png'))
        ...
        self.game_settings = {'Texts': {self.text.name: {'line': self.text.position}}} # initialise the variable
        ...
        # Text name
        self.textname = tk.Label(root, text=f"Text Name: \n\"{self.text.name}\"", font=textname_style)
        ...
        self.text_box = tk.Text(root,
                                height=self.display_nlines, width=int(self.width * 0.055),
                                padx=5, pady=5, font=text_style)
        self.text_box.grid(column=0, row=1, columnspan=3)
        self.text_box.config(wrap='word')
        self.draw_textbox()

        # user input
        self.userinput_box = tk.Text(root,
                                     height=4, width=int(self.width * 0.055),
                                     padx=5, pady=5, font=text_style)
    ...
    def save_game(self):
        ...
        history is the summmary of each attempt made.
            ...
            - match
        """
        game_settings = {'Texts': {self.text.name: {'line': self.text.position}}, 'History': self.game_history}
    ...
    def set_state(self, new_state):
        ...
        if self.state == 'READY':
            ...
            self.user_input_full = ""
            self.draw_userinput()
            self.instructions['text'] = 'Press Enter to continue or CTRL+q to quit.'

        if self.state == 'GAME':
            # self.instructions['text'] = ''
            self.instructions['text'] = 'Press Enter to finish or Esc to exit.'
    ...
    def clear_game_history(self, TextName = 'All'):
        ...
        if TextName == 'All':
            ...
            self.game_history = {key:vals for key, vals in self.game_history.items() if vals['TextName'] != TextName}
    ...
    def calc_typing_stats(self):
        ...
        results['duration'] = round((results['duration'].seconds+results['duration'].microseconds/1E6), 3)  # round to the millisecond
        results = typing_score(self.user_input, self.text.get_current_line(), results['duration'])
        ...
        results_str = f"{grade} - Time: {results['duration']:.2f} s   Accuracy: {results['accuracy']:.2%}   Wpm: {results['wpm']:.2f}"
        self.instructions['text'] = results_str
        print(results_str)
    ...
    def draw_textbox(self):
        ...
        self.text_box.insert(1.0, nl.join(self.text.contents[pos:pos + self.display_nlines]))

        matches = compare_lines(guess=self.user_input, answer=self.text.contents[pos])
        correct_chars = len(''.join(['1' if val else '0' for val in matches]).split('0')[0])
        self.text_box.tag_add("correct", '1.0', f'1.{correct_chars:.0f}')
        self.text_box.tag_add("incorrect", f'1.{correct_chars:.0f}', f'1.{len(self.user_input):.0f}')
